      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
          ...
          {Object.keys(widgets).map((wKey) => (
            <button
              key={wKey}
              onClick={() => setWidgets(prev => ({ ...prev, [wKey]: !prev[wKey] }))}
              style={{
                fontSize: '0.7rem',
                padding: '0.3rem 0.6rem',
                border: '1px solid var(--border)',
                borderRadius: '50px',
                backgroundColor: widgets[wKey] ? 'var(--primary-glow)' : 'var(--bg-surface)',
                color: widgets[wKey] ? 'var(--primary)' : 'var(--text-secondary)',
                fontWeight: 600,
                cursor: 'pointer'
              }}
            >
