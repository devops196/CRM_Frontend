...
export const FloatingAIAssistant: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'chat' | 'writer' | 'summaries' | 'predictions'>('chat');
  const [chatQuery, setChatQuery] = useState('');
  const [chatLog, setChatLog] = useState<{ sender: 'user' | 'ai'; text: string; action?: React.ReactNode }[]>([
  ...
  const handleQuerySubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      let matchedAction: React.ReactNode = undefined;
...
Export ready in PDF/CSV format.`;
  ...
  const toggleVoice = () => {
    if (isRecording) {
      ...
      setIsRecording(true);
      // Simulate speech-to-text writing a query after 2 seconds
      setTimeout(() => {
