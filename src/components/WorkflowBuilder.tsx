...
export const WorkflowBuilder: React.FC = () => {
  ...
  const [newNodeType, setNewNodeType] = useState<'action' | 'condition' | 'delay'>('action');
  const [newNodeValue, setNewNodeValue] = useState('');

  const currentWorkflow = workflows.find((w) => w.id === selectedWorkflowId);

  // Available triggers/actions for customization dropdowns
  ...
  const handleAddNode = (e: React.FormEvent) => {
    ...
    if (newNodeType === 'condition') {
      nodeText = newNodeValue;
    } else if (newNodeType === 'delay') {
      nodeText = newNodeValue;
    } else {
      nodeText = newNodeValue;
    }
            ...
            <form onSubmit={handleAddNode} style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: '0.5rem' }}>
                <select
                  ...
                  value={newNodeType}
                  onChange={(e) => {
                    setNewNodeType(e.target.value as any);
                    setNewNodeValue('');
                  }}
