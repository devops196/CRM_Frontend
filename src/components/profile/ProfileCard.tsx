...
/**
 * Displays the authenticated user's profile pulling ALL data strictly from the DB.
 * Includes explicit Team Management (Search DB users, add to team, remove from team).
 */
const ProfileCard: React.FC<ProfileCardProps> = ({ authUser }) => {
  const [dbUser, setDbUser] = useState<TeamMember | null>(null);
  ...
  const handleSearchDbUsers = useCallback(async (q: string) => {
    ...
    try {
      ...
      // Exclude self and already added team members
