...
import { GOOGLE_CLIENT_ID } from '../../services/auth.service';
 ...
 * Falls back to a styled custom button when the GIS library is unavailable
 * (e.g., no Client ID configured yet).
 */
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({ onSuccess, onError }) => {
  ...
  useEffect(() => {
    ...
    // Attempt to render immediately if GIS is already loaded
    if (window.google?.accounts?.id) {
      renderButton();
    } else {
      // Poll briefly for the GIS script to finish loading
  ...
  if (!isConfigured) {
    // Fallback UI when Google Client ID is not yet configured
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        ...
        <div style={{
          ...
          <code style={{ fontFamily: 'var(--font-mono)' }}>src/services/auth.service.ts</code>
        </div>
      </div>
    );
  }
...
/** Inline Google "G" logo SVG — used in fallback button */
